	...
	then
		...
		then
			...
			then
				...
				if [[ -z "$anilistid" ]]
				then
					wget -q -O- --post-data '{ "query": "{ Media(idMal: '"$malid"') { id startDate { day month year } } }" }' -H "https://graphql.anilist.co/" --header "content-type: application/json" -O anilist-infos.json
